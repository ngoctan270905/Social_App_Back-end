...
sequenceDiagram
    ...
    participant API as Comment API (Router/Service)
    participant DB as MongoDB (Comments/Posts)
    participant NotifySvc as Notification Service
    participant ConnMgr as Connection Manager (WebSocket)
    participant WSClient as Client (WebSocket)

    rect rgb(230, 240, 255)
        note right of Client: FLOW 1: CREATE NEW COMMENT / REPLY
        Client->>API: POST /api/v1/comments/ (post_id, content, [parent_id])
        activate API
        API->>DB: Validate Post/Parent Comment
        DB-->>API: (Post/Parent exist)
        API->>DB: Insert new comment (comments collection)
        API->>DB: $inc posts.comments_count
        alt If parent_id exists
            API->>DB: $inc comments.reply_count (for parent)
        end
        API->>NotifySvc: Call create_and_notify (user_id_to_notify, event_data)
        activate NotifySvc
        NotifySvc->>DB: Save notification (notifications collection)
        NotifySvc->>ConnMgr: send_to_user (user_id_to_notify, notification_payload)
        activate ConnMgr
        ConnMgr->>WSClient: Send JSON payload via WebSocket
        deactivate ConnMgr
        deactivate NotifySvc
        API-->>Client: 201 Created (New Comment/Reply Info)
        deactivate API
        WSClient-->>Client: Display Real-time Notification
    end

    rect rgb(255, 250, 230)
        note right of Client: FLOW 2: GET ROOT COMMENTS
        Client->>API: GET /api/v1/posts/{post_id}/comments (pagination)
        activate API
        API->>DB: Query comments (post_id, parent_id=null)
        DB-->>API: Return list of root comments
        API-->>Client: 200 OK (List of Comments)
        deactivate API
    end

    rect rgb(230, 255, 230)
        note right of Client: FLOW 3: GET REPLIES FOR A COMMENT
        Client->>API: GET /api/v1/comments/{comment_id}/replies (pagination)
        activate API
        API->>DB: Query comments (parent_id=comment_id)
        DB-->>API: Return list of replies
        API-->>Client: 200 OK (List of Replies)
        deactivate API
    end